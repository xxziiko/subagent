...
**인자가 있는 경우** (`/create-pr develop`, `/create-pr main`):

- `develop`, `devlop` → base를 `develop`으로 설정
- `main` → base를 `main`으로 설정

**인자가 없는 경우** (`/create-pr`):

- 사용자에게 질문: "어떤 브랜치를 base로 PR을 생성할까요?"
- 선택지: `develop`, `main`
  → 답변 전까지 진행 금지
