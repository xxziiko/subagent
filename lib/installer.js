...
/**
 * Install selected agents with category-aware path routing.
 *   general agents  → userDir/.claude/agents/   (~/.claude/agents/)
 *   project agents  → targetDir/.claude/agents/  (.claude/agents/)
 *
 * @param {string} sourceDir  - Source repo/package directory
 * @param {string} targetDir  - Project directory (for project-scoped agents)
 * @param {string} userDir    - User home directory (for general agents, e.g. os.homedir())
 * @param {string[]} agentNames
 * @param {{ copy?: boolean }} options
 */
async function installAgents(sourceDir, targetDir, userDir, agentNames, { copy = false } = {}) {
  const projectAgentsDir = path.join(targetDir, '.claude', 'agents');
  ...
  for (const name of agentNames) {
    ...
    // Route by category: general → user-level, project → project-level
    const destDir = agent.isProject ? projectAgentsDir : userAgentsDir;
    await fs.ensureDir(destDir);

    const destFile = path.join(destDir, `${name}.md`);
    const scope = agent.isProject ? 'project' : 'user';
    ...
    if (copy) {
      ...
      const relativeSrc = path.relative(destDir, agent.filePath);
      await fs.symlink(relativeSrc, destFile);
    }

    console.log(`  ${copy ? 'Copied' : 'Linked'} agent [${scope}]: ${name}`);
  }
}
