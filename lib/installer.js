...
const REPO_URL = 'https://github.com/your-org/subagent.git'; // TODO: 실제 레포 URL로 변경
const SUBAGENT_DIR = '.subagent';

/**
 * Discover available agents and skills from a source .claude/ directory.
 */
async function discoverItems(sourceDir) {
  ...
  for (const file of agentFiles) {
    ...
    const frontmatter = await parseAgentFrontmatter(path.join(agentsDir, file));
    agents.push({
      name,
    ...
    if (entry.isDirectory()) {
      ...
      if (await fs.pathExists(skillMd)) {
        skills.push({
          name: relativePath,
          type: 'directory',
          isProject: relativePath.startsWith('project'),
        });
      } else {
...
/**
 * Resolve all skill dependencies for selected agents.
 */
async function resolveSkillDeps(sourceDir, agentNames) {
  const deps = new Set();

  for (const name of agentNames) {
    const agentPath = path.join(sourceDir, '.claude', 'agents', `${name}.md`);
    const skillDeps = await getAgentSkillDeps(agentPath);
...
async function installAgents(sourceDir, targetDir, agentNames, { copy = false } = {}) {
  ...
  for (const name of agentNames) {
    const srcFile = path.join(sourceDir, '.claude', 'agents', `${name}.md`);
    ...
    if (copy) {
      ...
      const relativeSrc = path.relative(targetAgentsDir, srcFile);
